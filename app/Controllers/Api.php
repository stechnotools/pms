...
use Admin\Forms\Models\InstitutionModel;
...
class Api extends ResourceController
{
	...
	public function index(){
		$data['message'] = "Welcome to Integrated Farming under apps";
	...
	public function dashboarddata(){
        ...
        $data['fishery']=$fisheryModel->getTotal();

        $livestockModel=new LivestockModel();
        $data['livestock']=$livestockModel->getTotal();

        $institutionModel=new InstitutionModel();
        $data['institution']=$institutionModel->getTotal();
		...
		if($households){
			foreach($households as $key=>$result){
				...
				$json['households'][]=[
					...
					'date'=>date("Y-m-d H:i:s",strtotime($result['__system']['submissionDate'])),
    ...
    private function generateGVSItemset($filter){
        ...
        //$fp = fopen('php://temp', 'r+');
        $fp = fopen("php://output", "wb");
        $header=['list_name','name','label','district','block','grampanchayat','designation','nodal'];
        fputcsv($fp,$header);
        ...
        //state nodal person
        $statenodallist=$staffModel->getAll(['filter_nodal_state'=>1]);

        $statenodal=[];
        fputcsv($fp, $statenodal);
        foreach($statenodallist as $list){
            $staff_id=$list->id;
            $statenodal=['stateperson',$staff_id,$list->name,'','','',$list->designation,'state'];
            fputcsv($fp, $statenodal);
        }

        //district nodal person
        $districtnodallist=$staffModel->getAll(['filter_nodal_district'=>$filter['filter_district']]);
        $districtnodal=[];
        fputcsv($fp, $districtnodal);
        foreach($districtnodallist as $list){
            $staff_id=$list->id;
        $districtnodal=['districtperson',$staff_id,$list->name,'','','',$list->designation,'district'];
        $districtnodal=['districtperson',$staff_id,$list->name,'','','',$list->designation,'district'];
            fputcsv($fp, $districtnodal);
        }
