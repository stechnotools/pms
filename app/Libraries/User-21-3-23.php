...
class User
{
    ...
    public function hasPermission($data) {
        //echo $data."<br>";
        $subUrl = ['add','edit','view','delete'];
        $hash_permission=false;

        if(strpos($data, '_edit') == false && strpos($data, '_view') == false && strpos($data, '_delete') == false && strpos($data, '_add') == false ) {
            $hash_permission=true;
        }
    ...
    public function getImage() {
        ...
        } else if($this->image && strpos($this->image,'http')!==false) {
            $photo = $this->image;
        } else {
