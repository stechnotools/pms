...
class Autoload extends AutoloadConfig
{
    public function __construct(){
        ...
        foreach(glob(ROOTPATH . 'mobile/*', GLOB_ONLYDIR) as $item_dir)
        {
            $folder = pathinfo($item_dir, PATHINFO_FILENAME);
            $this->psr4["Mobile\\".$folder]=ROOTPATH . 'mobile/'.$folder;
        }
        foreach(glob(ROOTPATH . 'soe/*', GLOB_ONLYDIR) as $item_dir)
        {
            $folder = pathinfo($item_dir, PATHINFO_FILENAME);
            $this->psr4["Soe\\".$folder]=ROOTPATH . 'soe/'.$folder;
        }
